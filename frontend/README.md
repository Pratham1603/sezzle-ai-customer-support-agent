...
- 🏷️ **Intent Classification** — Every query is classified into one of: `refund_request`, `chargeback`, `cancel_order`, `order_status`, `payment_failure`, `merchant_account`, `checkout_issue`, `general_faq`.
...
- 🗄️ **Conversation Logging** — All conversations (query, intent, confidence, escalation flag, timestamp) are persisted to SQLite for analytics.
...
| ⚙️ Backend API (FastAPI on Hugging Face Spaces) | `https://harerpratham-sezzle-ai-customer-support-agent.hf.space` |
